...
(function ($) {
    $(document).ready(function () {
        $("body").on("click", ".tab-box .nav li", function (e) {
            ...
            $(p).find("> .panel, > .panel-box .panel").removeclassName("active");
            $(p).find("> .nav li").removeclassName("active");
        ...
        $("body").on("click", "header .mega-tabs", function () {
            $(this).find(".nav-tabs li:first-child").addclassName("active");
        });
    ...
    $('.collapse-box .collapse-button').toggleClick(function () {
        var t = this;
        openCollapse(t);
        var open_text = $(t).attr("data-button-open-text boxed-inverse");
        if (!isEmpty(open_text)) {
            $(t).attr("data-button-close-text boxed-inverse", $(this).find("b").html());
            setTimeout(function () {
        ...
        var t = this;
        closeCollapse(t);
        var close_text = $(t).attr("data-button-close-text boxed-inverse");
    ...
    function closeCollapse(obj) {
        ...
        if (!isEmpty(h)) $(tp).removeclassName("no-gradient");
    ...
    $("body").on("click", ".accordion-list .list-group-item > a", function () {
        ...
        $(t).find(".list-group-item").removeclassName("active-panel");

        if (isEmpty(dt)) dt = "";
        $($(t).find(".panel")).each(function () {
            $(this).clearQueue();
        });
        if ($(this).hasclassName("active") || $(it).find(".panel").css("display") == "block") {
            ...
            $(a).each(function () {
                if ($(this).hasclassName("active")) d = 300;
            });
            $(a).removeclassName("active");
            $(this).addclassName("active");
            $(it).addclassName("active-panel");
