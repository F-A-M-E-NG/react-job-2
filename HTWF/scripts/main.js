...
$(() => {
  ...
  $("#login-form").on("submit", e => {
    ...
    $.ajax({
      ...
      error: res => {
        ...
        $("#login-alert").addclassName("alert-danger");
        $("#login-alert").removeclassName("alert-success");
        if (res.statusText === "error") {
          ...
          if (
            ...
            $(".alert").removeclassName("alert-danger");
      ...
      $("#login-alert").addclassName("alert-success");
      $("#login-alert").removeclassName("alert-danger");
  ...
  $("#password-reset-form").on("submit", e => {
    ...
    $.ajax({
      ...
      contentType: "application/json",
      error: res => {
        $("#password-reset-alert").addclassName("alert-danger");
        $("#password-reset-alert").removeclassName("alert-success");
        if (res.statusText === "error") {
      ...
      $("#password-reset-alert").addclassName("alert-success");
      $("#password-reset-alert").removeclassName("alert-danger");
      $("#password-reset-alert").text(
  ...
  $("#register-form").on("submit", e => {
    ...
    $.ajax({
      ...
      error: res => {
        console.log(res);
        $("#register-alert").addclassName("alert-danger");
        $("#register-alert").removeclassName("alert-success");
        if (res.statusText === "error") {
          $("#register-alert").text(
            "Oops! Something went wrong, It's not you, it's us. Please try again later."
          );
      ...
      $("#main-form").hide();
      $("#register-alert").addclassName("alert-success");
      $("#register-alert").removeclassName("alert-danger");
      $("#register-alert").text(
  ...
  $("#otp-form").on("submit", e => {
    ...
    $.ajax({
      ...
      timeout: 15000,
      error: res => {
        $(".alert").addclassName("alert-danger");
        $(".alert").removeclassName("alert-success");
        if (res.statusText === "error") {
          $(".alert").text(
            "Oops! Something went wrong, It's not you, it's us. Please try again later."
          );
    ...
    $.ajax({
      ...
      const accounts = data.map(
        ...
        <div className="col-md-4">
          <div className="advs-box boxed-inverse shadow-1">
            <h3>${c.accountType}<br><small>${c.accountNumber}</small></h3>
            <a href="/dashboard/plan/?accountNumber=${c.accountNumber}" className="btn btn-primary">View More</a>
          </div>
        </div>
        `
      );
